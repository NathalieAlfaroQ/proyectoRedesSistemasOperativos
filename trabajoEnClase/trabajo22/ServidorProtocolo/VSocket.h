...
class VSocket {
   public:
      ...
      void EnableUDP(); // Habilitar UDP
      size_t SendUDP(const void* buffer, size_t len, const char* ipAddress, int port); // Enviar mensajes UDP
      size_t ReceiveUDP(void* buffer, size_t len, char* ipAddressOutput, int* portOutput); // Recibir mensajes UDP
      int bindSocketToInterface(const char* interface_name);

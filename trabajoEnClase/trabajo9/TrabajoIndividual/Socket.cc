...
void Socket::SSLCreate( Socket * original ) {
   ...
   /*Por que tengo que hacer otra vez SSL_new para despues hacer SSL_set_fd si la estructura de datos que
   contiene al contexto ya esta guardada en this->SSLStruct?*/
   // ssl = SSL_new( (SSL_CTX *) this->SSLContext );
   // por que esta funcion debe adjuntar el socket original al sslstruct, no deberia ser el socket
   // devuelto por accept el que debe adjuntarsele?
...
void Socket::SSLInitContext() {
   ...
   */
   // Se crea la estructura de datos con la info antes mencionada
...
int Socket::SSLConnect( const char * hostName, int port ) {
   ...
   // std::cout << " antes de hacer set fd: this->SSLStruct: " << this->SSLStruct << std::endl;
...
void Socket::SSLInitServerContext() {
   /*
   Inicializa estructuras de datos internas, carga de algoritmos 
   criptograficos y configuracion de parametros de seguridad. Siempre
   se llama al inicio. 
   */
   SSL_library_init();

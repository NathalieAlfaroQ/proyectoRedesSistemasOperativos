...
int VSocket::Listen( int queue ) {
   /* se crea una cola de conexiones de maximo 'queue' manejada por el sistema operativo
    * se marca el socket idSocket como pasivo
    * pasivo implica que se encarga de aceptar solicitudes de conexion entrantes usando accept
   */
...
int VSocket::Bind( int port ) {
   ...
   if (this->IPv6)
   {
      ...
      /*La persona que llama solicita que el socket este vinculado
      a todas las interfaces de red en el host
      */
      ...
      // Convierte de little a big endian
      addrIpv4.sin_port = htons (port);
      // Acepta conexiones dirigidas hacia cualquier interfaz de red en el servidor
      // Por ejemplo la interfaz de red inalambrica o a traves de ethernet
